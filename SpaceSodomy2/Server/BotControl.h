...
public:
    BotControl(std::string bot_name, std::string team_name, ShipBrain::Type type, const GameReadable&);
    BotControl(BotControl&&) = delete;
    BotControl& operator=(BotControl&&) = delete;
    ~BotControl();
    void set_equip(std::string bot_name, std::string team_name, ShipBrain::Equip equipment);


    // The bot is lazy : if a message is not needed, it does not compute action
