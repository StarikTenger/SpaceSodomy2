...
void Bar::step() {
	...
	get_draw()->fill_rect(get_pos() - get_screen_mode() * b2Vec2(get_scale().x * (((max_value == nullptr) ? 0 : *max_value)
		- ((value == nullptr)? 0 : *value)) / ((max_value == nullptr) ? 0 : *max_value) / 2 , 0),
		get_screen_mode() * (get_scale() - b2Vec2(get_scale().x * (((max_value == nullptr)? 0 : *max_value)
			- ((value == nullptr) ? 0 : *value)) / ((max_value == nullptr) ? 0 : *max_value), 0)), front_color, angle);
	if (draw_text) 
		text.step();
}